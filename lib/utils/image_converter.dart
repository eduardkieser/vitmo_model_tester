...
class ImageConverter {
  static Future<List<List<imglib.Image>>> convertCopyRotateSetFast(
      Map<String, dynamic> convertCropData) async {
    ...
    cropData['frames'].forEach((LensModel frame) {
      if (frame.isMMM) {
        ...
        double frameWidth =
            (frame.secondCorner.dx - frame.firstCorner.dx).abs();
        double frameHeight =
            (frame.secondCorner.dy - frame.firstCorner.dy).abs();
        ...
        int x1 = ((frame.firstCorner.dx + frameWidth * subFrameSizeFactor) /
                screenSize[0] *
                image.width)
            .round();
        int y1 = ((frame.firstCorner.dy + frameHeight * subFrameSizeFactor) /
                screenSize[1] *
                image.height)
            .round();
        imglib.Image imageOut1 =
            imglib.copyCrop(image, x0, y0, (x1 - x0), (y1 - y0));
        imageOut1 = imglib.copyResize(imageOut1, width: 48, height: 48);

        /// Top right subframe(Usually Min)
        x0 = ((frame.secondCorner.dx - frameWidth * subFrameSizeFactor) /
                screenSize[0] *
                image.width)
            .round();
        y0 = (frame.firstCorner.dy / screenSize[1] * image.height).round();
        x1 = (frame.secondCorner.dx / screenSize[0] * image.width).round();
        y1 = ((frame.firstCorner.dy + frameHeight * subFrameSizeFactor) /
                screenSize[1] *
                image.height)
            .round();
        imglib.Image imageOut2 =
            imglib.copyCrop(image, x0, y0, (x1 - x0), (y1 - y0));
        imageOut2 = imglib.copyResize(imageOut2, width: 48, height: 48);

        /// Bottom Center, usually mean
        x0 = ((frame.firstCorner.dx +
                    (frameWidth / 2) * (1 - subFrameSizeFactor)) /
                screenSize[0] *
                image.width)
            .round();
        y0 = ((frame.secondCorner.dy - frameHeight * subFrameSizeFactor) /
                screenSize[1] *
                image.height)
            .round();
        x1 = ((frame.firstCorner.dx +
                    (frameWidth / 2) * (1 + subFrameSizeFactor)) /
                screenSize[0] *
                image.width)
            .round();
        y1 = (frame.secondCorner.dy / screenSize[1] * image.height).round();
        imglib.Image imageOut3 =
            imglib.copyCrop(image, x0, y0, (x1 - x0), (y1 - y0));
  ...
  static Future<List<imglib.Image>> cropRotateSet(
